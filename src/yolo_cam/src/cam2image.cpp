...
namespace cam2image_vm2ros
{
  ...
  void Cam2Image::parse_parameters()
  {
    ...
    remote_timeout_ = this->declare_parameter("remote_timeout", 15);
...
int main(int argc, char * argv[])
{
    ...
    auto cam_node = std::make_shared<cam2image_vm2ros::Cam2Image>(rclcpp::NodeOptions());
    auto image_proc_node = std::make_shared<image_processing_vm2ros::ImageProcessor>(rclcpp::NodeOptions());
    // auto object_pos_node = std::make_shared<object_position_vm2ros::ObjectPosition>(rclcpp::NodeOptions());


    // Add nodes to executor
    // exec.add_node(relbot_simulator);
    exec.add_node(cam_node);
    exec.add_node(image_proc_node);
    // exec.add_node(object_pos_node);
